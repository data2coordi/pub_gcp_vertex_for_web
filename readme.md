...
# ##最終アウトプットのサンプルイメージ
下記は日経平均株価のある掲示板を本システムで解析した結果である。

![Bigqueryでの検索イメージ](./img/analyze_ret.jpg)

上記の左側の列が分類と要約結果。右側が元の投稿。
...
下記はLooker studioとBigqueryのVIEWを使ってデータをレポート化して表示したもの
VertexAIによりスパムとその他に分類されたものが半数近くを占める。これらをスキップして読むことで時間が短縮できる。
