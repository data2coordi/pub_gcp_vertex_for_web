...
上記の左側の列がVertex AIが出力した投稿の分類と要約。右側が元の投稿。

SQLで**スパム**に分類された投稿は非表示にしている。

上記のように投稿を分析した結果にって、ユーザーの掲示板の利用をサポートする。




下記はLooker studioとBigqueryのVIEWを使ってデータをレポート化して表示したものである。
分類ごとのカウントから関心が強い分類の多い掲示板かわかる。
![投稿の分類イメージ](./img/提示版分類集計.jpg)
**スパム**と分類したものはスキップする、関心のある分類の投稿にフォーカスして読む等、効率的に掲示板を利用できる。
